...
<body>
  ...
  <main class="FondoDueñoAdministrador" aria-label="Panel de gestión de promociones">
    <div class="container-fluid filtraderos justify-content-end" aria-label="Acciones de gestión">
      <a href="CrearPromocion.php"><button class="btn btn-success" aria-label="Crear nueva promoción"><i class="bi bi-plus-circle"></i> Crear </button></a>
      <form method="GET" class="d-inline" aria-label="Ordenar promociones">
        <select name="orden" class="form-select bg-primary text-white d-inline w-auto mx-2" onchange="this.form.submit()" aria-label="Seleccionar orden de promociones">
    ...
    ?>
    <div class="promociones" aria-label="Listado de promociones">
      <?php if ($promociones && mysqli_num_rows($promociones) > 0): ?>
        <?php while ($promo = mysqli_fetch_assoc($promociones)): ?>
          <div class="promocion" aria-label="Tarjeta de promoción">
            ...
            <div class="acciones" aria-label="Acciones de la promoción">
              <button type="button" class="btn btn-danger Eliminar" data-bs-toggle="modal" data-bs-target="#modalEliminar" data-cod="<?php echo $promo['cod_promocion']; ?>" aria-label="Eliminar promoción">
                ELIMINAR
              </button>
            </div>
          </div>
        <?php endwhile; ?>
      <?php else: ?>
        <p aria-label="Sin promociones">No hay promociones registradas.</p>
      <?php endif; ?>
    </div>
