<?php
// 1. ACTIVAR BÚFER DE SALIDA: Esto evita el error "headers already sent"
ob_start(); 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['imagen'])) {
        ...
        if ($errorSubida === UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, $permitidos)) {
                ...
                if (!is_dir("../uploads/")) {
                    $mensaje = "<div class='alert alert-danger'>La carpeta '../uploads/' no existe.</div>";
                } else if (move_uploaded_file($_FILES['imagen']['tmp_name'], $destino)) {
                    $rutaMultimedia = "uploads/" . $nuevoNombre;
                } else {
                    $mensaje = "<div class='alert alert-danger'>Error al guardar imagen.</div>";
                }
            }
        }
    }
    ...
    if (!$mensaje && ($vTitulo === '' || $vFechaInicio === '' || $vFechaFin === '' || $vCodLocal === '' || empty($diasSeleccionados) || $rutaMultimedia === '')) {
        $mensaje = "<div class='alert alert-danger'>Complete todos los campos obligatorios e imagen.</div>";
    }
        ...
        if (mysqli_query($link, $sql)) {
            ...
            if ($idPromocion > 0) {
                ...
                if (consultaSQL($sqlDias)) {
                    $mensaje = "<div class='alert alert-success'>La promoción fue creada. Redirigiendo...</div>";
                    // 2. REDIRECCIÓN POR JAVASCRIPT: Evita colisiones con las cabeceras de PHP
                    echo "<script>
                            setTimeout(function(){ 
                                window.location.href = 'SeccionDueñoLocal.php'; 
                            }, 2000);
                          </script>";
                } else {
